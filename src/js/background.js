window.bubbly = function(config) {
 ...
 bubbly({
   ...
   blur: 4, // default is 4
   bubbleFunc: () =>
     `hsla(${Math.random() * 360}, 20%, 30%, ${Math.random() * 0.15})`, // default is () => `hsla(0, 0%, 100%, ${r() * 0.1})`)
   bubbles: 100, // default is Math.floor((canvas.width + canvas.height) * 0.02);
   canvas: document.querySelector('#background'), // default is created and attached
   colorStart: 'rgb(255, 255, 255)', // default is blue-ish
   colorStop: 'rgb(212, 243, 251)', // default is blue-ish
